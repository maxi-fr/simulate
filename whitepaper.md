...
* **State and conventions:** The state vector is `x = [r(3) | v(3) | q(4) | ω(3) | effector states...]`. Position `r` and velocity `v` are expressed in the inertial frame; the attitude quaternion `q` is a scalar-last JPL quaternion (`[x, y, z, w]`) representing the body→inertial rotation; and angular velocity `ω` is in the body frame. The equations of motion follow Newton–Euler: `v̇ = (1/m)·F` and `ω̇ = J⁻¹·(τ − ω × (J·ω + h))`, with quaternion kinematics `q̇ = ½·Ω(ω)·q`. The summed effector force `F` is applied directly in the translational equation; the dynamics kernel itself adds neither a body→inertial rotation nor a gravity term — central two-body gravity is supplied, when desired, as a command-free `EarthGravity` effector (`F = −μ·m·r/|r|³`), which keeps the integrated orbit Keplerian.
...
However, advanced algorithms require tracking specialized internal variables. To bridge the gap between dynamic internal states and strict type-safety, **components must define their logging schema using frozen dataclasses**. These component-specific logs must only be used to record specialized internal values (such as integrator accumulations, filter error signals, or noise parameters) and should never duplicate signals that are already logged universally.
