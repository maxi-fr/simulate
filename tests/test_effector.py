...
def _state(r: np.ndarray, q: np.ndarray) -> BodyState:
    ...
    return BodyState(r=r, v=np.zeros((3, 1)), q=q, omega=np.zeros((3, 1)))
...
def test_zero_torque_at_principal_axis_equilibrium() -> None:
    ...
    out = gg.evaluate(0.0, _state(r, q), np.zeros((0, 1)), np.zeros((0, 1)))
...
def test_torque_scales_as_inverse_r_cubed() -> None:
    ...
    near = gg.evaluate(0.0, _state(np.array([[7.0e6], [0.0], [0.0]]), q), np.zeros((0, 1)), np.zeros((0, 1)))
    far = gg.evaluate(0.0, _state(np.array([[1.4e7], [0.0], [0.0]]), q), np.zeros((0, 1)), np.zeros((0, 1)))
