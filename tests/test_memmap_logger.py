...
from simulate.logger import MmapLogger, RamLogger
...
def test_logger_export_memmap(tmp_path: Path) -> None:
    """Test that a memory-mapped run packs component signals into {prefix}.npz."""
    logger = MmapLogger(total_steps=1, directory=tmp_path, prefix="test")

    logger.log(0.1, {"comp1": MockComponentLog(value=42.0, vec=np.array([1.0, 2.0]))})
...
def test_logger_export_multiple_rows(tmp_path: Path) -> None:
    ...
    logger.log(0.0, {"comp1": MockComponentLog(value=10.0, vec=np.array([0.0]))})
    logger.log(1.0, {"comp1": MockComponentLog(value=20.0, vec=np.array([1.0]))})
    logger.finalize(tmp_path, prefix="test")
...
def test_logger_finalize_raises_on_partial_fill(tmp_path: Path) -> None:
    ...
    for i in range(3):
        logger.log(float(i), {"comp1": MockComponentLog(value=float(i), vec=np.array([float(i)]))})
...
def test_logger_export_large_multidim(tmp_path: Path) -> None:
    ...
    for i in range(n):
        logger.log(float(i), {"dynamics": MockComponentLog(value=float(i), vec=np.full(x_shape, float(i)))})
    ...
    data = np.load(npz_file)
    assert data["dynamics.vec"].shape == (n, 6, 76)
    for i in range(n):
        assert np.all(data["dynamics.vec"][i] == float(i))
...
def test_memmap_matches_ram(tmp_path: Path) -> None:
    ...
    for i in range(5):
        mm_logger.log(float(i), {"comp1": MockComponentLog(value=float(i), vec=np.array([float(i), float(i)]))})
    mm_logger.finalize(tmp_path, prefix="mm")
    mm_data = np.load(tmp_path / "mm.npz")

    ram_logger = RamLogger(total_steps=5)
    for i in range(5):
        ram_logger.log(float(i), {"comp1": MockComponentLog(value=float(i), vec=np.array([float(i), float(i)]))})
