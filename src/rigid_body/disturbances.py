...
def gravity_gradient(r_eci: np.ndarray, q_bi: Quaternion, J_B: np.ndarray) -> np.ndarray:
    """
    Calculate the gravity gradient torque on a satellite.
    ...
    nadir_inertial = -r_eci / r_norm
    o_body = q_bi.apply(nadir_inertial)

    return (3.0 * MU / r_norm**3) * np.cross(o_body, J_B @ o_body)
