"""Reference-frame kinematics: the orbital (LVLH) frame, Euler angles, and orbital rate.

The orbital reference frame (LVLH, a local-vertical/local-horizontal frame) used here has the
following axes, expressed in the inertial (ECI) frame::

    z_lvlh = -r / |r|                  (nadir, points toward the central body)
    y_lvlh = -(r x v) / |r x v|        (negative orbit normal, the pitch axis)
    x_lvlh =  y_lvlh x z_lvlh          (along-track, the roll axis)

A nadir-pointing body aligns its body frame with this frame, so the desired attitude is the
inertial->LVLH rotation (consistent with the ``q_bi`` convention used elsewhere, where
``q_bi.apply`` maps an inertial vector into the body frame) and the desired body rate is the
LVLH frame's angular velocity expressed in LVLH coordinates.
"""
...
def _eci_to_lvlh_matrix(r_eci: np.ndarray, v_eci: np.ndarray) -> np.ndarray:
    """Build the inertial->LVLH rotation matrix whose rows are the LVLH axes in ECI."""
...
def lvlh_from_orbit(r_eci: ArrayLike, v_eci: ArrayLike) -> Quaternion:
    """Nadir-pointing attitude (inertial->LVLH) for a given orbit state.
    ...
    -------
    Quaternion
        The rotation ``q`` such that ``q.apply(v_eci) = v_lvlh``, i.e. a body frame aligned
        with this quaternion points its ``z`` axis at nadir.
    """
    matrix = _eci_to_lvlh_matrix(np.asarray(r_eci, dtype=float), np.asarray(v_eci, dtype=float))
    return Quaternion.from_scipy(Rotation.from_matrix(matrix))


def orbital_rate(r_eci: ArrayLike, v_eci: ArrayLike) -> Vec3:
    """Angular velocity of the LVLH frame, expressed in LVLH coordinates.
    ...
    motion for a circular orbit); it is rotated into the LVLH frame before being returned.
    ...
    return _eci_to_lvlh_matrix(r, v) @ omega_eci
...
def eci_attitude_from_lvlh(  # noqa: PLR0913
    r_eci: ArrayLike,
    v_eci: ArrayLike,
    *,
    roll: float,
    ...
    """Inertial attitude and body rate from an attitude expressed relative to the LVLH frame.
    ...
    roll, pitch, yaw : float
        Body attitude relative to the LVLH frame as intrinsic ``Y-X-Z`` Euler angles.
    omega_bo : ArrayLike
        Body angular velocity relative to the LVLH frame, expressed in the body frame, shape ``(3,)``.
    degrees : bool, optional
    ...
    """
    q_bo = quaternion_from_euler([pitch, roll, yaw], degrees=degrees)  # LVLH->body
    q_li = lvlh_from_orbit(r_eci, v_eci)
    q_bi = q_bo * q_li
